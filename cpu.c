    ...
    { switch (CPU.IRopcode)
      { case OPload:
        ...
        case OPifgo:  // conditional goto, need two instruction words
          ...
          if (Debug) printf ("Process %d is executing: Goto %d, If %d,%f is greater than 0\n", CPU.Pid,  gotoaddr, CPU.IRoperand, CPU.MBR);
          if (CPU.MBR > 0) CPU.PC = gotoaddr - 1;// Note: PC will be increased by 1, Hence set to 1 less
          break; 
        case OPstore:
        ...
        case OPprint:
         ...
         sprintf (str, "PID=%d M[%d]=%.2f", CPU.Pid, CPU.IRoperand, CPU.MBR);
         insert_termio (CPU.Pid, str, regularIO, PCB[CPU.Pid]->sockfd);
         CPU.exeStatus = eWait;
          break;
        case OPsleep:
