...
{ 
  ...
  PCB[pid]->PC = CPU.PC;
  PCB[pid]->AC = CPU.AC;
  PCB[pid]->exeStatus = CPU.exeStatus;  //Context Out Doesnot Need MBR,MAR etc to be saved which are Temporary Registers
}
...
int pid;
{ PCB[pid]->exeStatus = CPU.exeStatus; //sockfd = PCB[pid]->sockfd;
...
{ int pid;
  ...
  { 
    ...
    context_in (idlePid);
    CPU.exeStatus = eRun;
    add_timer (idleQuantum, CPU.Pid, actTQinterrupt, oneTimeTimer); //idleQuantum is Given in Config.Sys 
    cpu_execution ();
